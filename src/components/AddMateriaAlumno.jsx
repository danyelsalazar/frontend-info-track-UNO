...
const AddMateriaAlumno = ({setAbrirAddMateria, abrirAddMateria}) => {
  ...
  return (
    <div className= {`container-add-materia-alumno-none ${!abrirAddMateria ? '' : 'container-add-materia-alumno'}`} >
      <form
        className="form-add-tasks form-add-tasks"
        action=""
        onSubmit={handleSubmit}
      >
        <h2 className="title-form-task">Carga los datos de tu materia</h2>
        {/* SELECT CARRERA */}
        <div className="container-selects-form">
          <div className="container-select-label">
            ...
            >
              ...
              {careers.map((carrera) => (
                <option
                  key={carrera.id}
                  value={JSON.stringify({ name: carrera.name, id: carrera.id })}
                >
                  {carrera.name}
                </option>
              ))}
          ...
          <div className="container-select-label">
            ...
            >
              <option value="">---</option>
              {subjects
                .filter((materia) => materia.careerId === form.carrera.id)
                .map((materia) => (
          ...
          <div className="container-select-label">
            {/* SELECT ESTADO */}
            <label htmlFor="selector-estado">
              Selecciona el estado de la materia
            </label>
            <select
              id="selector-estado"
              name="estado"
        ...
        <div className="btns-form-add-task">
            <button type="button" className="btn-cancelar-add-task  button-ingresar" onClick={()=>{setAbrirAddMateria(false)}}>Cancelar</button>
        <button type="submit" className="btn-form-task button-ingresar">
          Cargar materia
        </button>
        </div>
      </form>
