...
export const MiMateria = ({ materia, onEditar }) => {
  const [removeMateria, { loading: loadingRemove }] =
    useMutation(REMOVE_MATERIA);
  const { setMaterias } = useAuthContext();

  const handleRemove = async () => {
    const { data } = await removeMateria({
      variables: { materiaId: materia.materia.id },
    });
      ...
      <main className="materia-user-content">
        ...
        {materia.estado === "REGULARIZADA" && (
          <>
            ...
            <p className="nota-materia">
              Llamados usados:{" "}
              <b className="number-nota">{`${materia.llamadosUsados}/3`}</b>
            </p>
          </>
        )}
